...
<nav class="nav">
  ...
  <div class="nav-links">
      ...
      <?php if (!empty($item['dropdown'])): ?>
        <div class="nav-item has-dropdown">
          <a href="<?= View::url($item['url']) ?>" class="nav-link <?= $active === $item['key'] ? 'is-active' : '' ?>">
            <?= View::e($item['label']) ?> <span class="caret">▼</span>
          </a>
          <div class="dropdown">
            <?php if ($item['dropdown'] === 'servicio'): ?>
              <span class="dropdown-title">Mantenimiento e instalación de:</span>
              <?php foreach ($servicioSub as $sub): ?>
                <a href="<?= View::url('servicio') ?>" class="dropdown-item">• <?= View::e($sub) ?></a>
              <?php endforeach; ?>
            <?php else: ?>
              <?php foreach ($solucionSub as $sub): ?>
                <a href="<?= View::url('soluciones') ?>" class="dropdown-item">• <?= View::e($sub) ?></a>
              <?php endforeach; ?>
            <?php endif; ?>
          </div>
        </div>
    ...
    <div class="lang-switch">
      <a data-lang="es" href="#" title="Español"><img src="https://flagcdn.com/24x18/es.png" alt="ES"> ES</a>
      <a data-lang="en" href="#" title="English"><img src="https://flagcdn.com/24x18/gb.png" alt="EN"> EN</a>
    </div>
  </div>
</nav>
